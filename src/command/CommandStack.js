...
draw2d.command.CommandStack = Class.extend(
  /** @lends draw2d.command.CommandStack.prototype */
  {
    ...
    execute: function (command) {
      ...
      this.notifyListeners(
        command,
        draw2d.command.CommandStack.PRE_EXECUTE,
        "PRE_EXECUTE"
      );
      ...
      if (this.undostack.length > this.maxundo) {
        this.undostack = this.undostack.slice(
          this.undostack.length - this.maxundo
        );
      }
      this.notifyListeners(
        command,
    ...
    off: function (listener) {
      this.eventListeners = this.eventListeners.grep(
        (entry) => !(entry === listener || entry.stackChanged === listener)
      );
      return this;
    },
    ...
    notifyListeners: function (command, state, action) {
      let event = new draw2d.command.CommandStackEvent(
        this,
        command,
        state,
        action
